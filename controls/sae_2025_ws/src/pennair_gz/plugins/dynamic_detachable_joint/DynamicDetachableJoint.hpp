...
public:
  DynamicDetachableJoint() = default;
...
private:
  transport::Node::Publisher outputPub;
...
private:
  void PublishJointState(bool attached);
...
private:
  std::string attachedChildModelName;

private:
  std::string attachedChildLinkName;

  /// \brief Minimum distance to consider the child link as attached

private:
  double defaultAttachDistance = 0.1;

  /// \brief Distance to consider the child link as attached

private:
  double attachDistance{defaultAttachDistance};
