        ...
        sys.stdout.write(
            "\n".join(
                [
                    ...
                    "├─────────────────────────────────────────────────┤",
                    f"│  drive : {self.topic:<40s}│",
                    f"│  servo : {self.servo_topic:<40s}│",
                    f"│  cmd   : lin={linear:+6.2f}  ang={angular:+6.2f}                │",
                    f"│  servo : {self.servo_degree:6.1f} deg  (step {self.servo_step:.1f})              │",
                    "├──────────────┬──────────────────────────────────┤",
                    "│  Movement    │  Speed / Servo Adjust            │",
                    f"│  {k('w')}  forward  │  {k('m')} / {k('M')}  linear  +/- 0.1  {self.linear:.2f}│",
                    f"│  {k('s')}  reverse  │  {k('n')} / {k('N')}  angular +/- 0.1  {self.angular:.2f}│",
                    f"│  {k('a')}  left     │  {k('[')} / {k(']')}  servo   +/- step        │",
                    f"│  {k('d')}  right    │  {k('p')} / {k('P')}  servo step +/- 5.0      │",
                    f"│              │  Ctrl-C to quit                  │",
                    "└──────────────┴──────────────────────────────────┘",
                ]
            )
