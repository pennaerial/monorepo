...
class HoopScoringNode(ScoringNode):
    ...
    def __init__(
        ...
        self.drone_publisher = self.create_publisher(
            Float32MultiArray, "/scoring/in_house/drone_position", 10
        )
    ...
    def load_hoop_positions(self):
        ...
        try:
            ...
            self.get_logger().info(
                f"Loaded {len(self.hoop_poses)} hoops from 'list_hoops' service."
            )
    ...
    def update_scoring(self) -> None:
        ...
        for i, hoop in enumerate(self.hoop_poses):
            ...
            inside_hoop = (
                abs(drone_pos_in_hoop[0]) <= hoop_radius
                and abs(drone_pos_in_hoop[2]) <= hoop_radius
            )  # x and z within radius
        ...
        if self.uav_position:
            self.get_logger().info(
                f"UAV Position: x={x:.2f}, y={y:.2f}, z={z:.2f}",
    ...
    def check_landing_bonus(self):
        ...
        if is_near_origin and is_low_enough:
            ...
            # Publish status and score
            self.publish_status(
                f"🏁 LANDING BONUS! +{self.landing_bonus} points! Total: {self.current_score:.1f}"
            )
            self.publish_score()

            # Log achievement
            self.get_logger().info(
                f"🏁 LANDING BONUS AWARDED! +{self.landing_bonus} points at origin (0,0)"
            )
            self.get_logger().info(f"Final Score: {self.current_score:.1f}")

    def _world_to_hoop_local(
        self,
        world_pos: Tuple[float, float, float],
        hoop_pos: Tuple[float, float, float],
        hoop_euler: Tuple[float, float, float],
