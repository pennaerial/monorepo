...
def test_normalize_stage_world_params_promotes_legacy_payloads():
    params = normalize_stage_world_params(
        {
            "template_world": "worlds/template.sdf",
            "payload_0": {"path_to_sdf": "payload.sdf"},
            "entities": {"marker": {"path_to_sdf": "marker.sdf"}},
        }
    )
    ...
    assert params["controllables"]["payload_0"]["kind"] == "payload"


def test_resolve_stage_world_defaults_to_base_stage():
...
def test_resolve_stage_world_loads_specific_stage_and_merges_world_overrides():
    ...
    assert resolved["world"]["params"]["dlz"]["path_to_sdf"].endswith("dlz_white/model.sdf")
    ...
    assert resolved["world"]["params"]["controllables"]["uav_0"]["position"] == [1.0, 2.0, 3.0]
