...
class BridgeNode(Node):
    ...
    def _send_to_peers(self, packet: bytes, label: str) -> None:
    ...
    def _on_out(self, msg, topic_id: int) -> None:
        ...
        self._send_to_peers(packet, f"data topic={topic_name!r}")
    ...
    def _handle_heartbeat(self, data: bytes, addr) -> None:
        ...
        self._debug(f"IN heartbeat from {addr[0]}:{sender_port} seq={seq}")
    ...
    def _heartbeat_loop(self) -> None:
        while not self._stop.is_set():
            ...
            self._send_to_peers(packet, f"heartbeat seq={self._heartbeat_seq}")
