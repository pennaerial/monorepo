...
void GPIOController::initialize(rclcpp::Node* node)
{
    ...
    const int pwm_hz =
        std::max(1, static_cast<int>(std::lround(p.motor.pwm_frequency)));
    ...
    servo_ = std::make_unique<Servo>(
        ...
        p.pins.SERVO,
        p.servo.frequency,
        p.servo.pulse_min_us,
        p.servo.pulse_max_us);
    ...
    motor_state_pub_ =
        node_->create_publisher<payload_interfaces::msg::MotorState>(
            state_topic, 10);
    zn_service_ = node_->create_service<
        payload_interfaces::srv::ComputePidZieglerNichols>(
        zn_service_name,
        [this](
            const std::shared_ptr<
                payload_interfaces::srv::ComputePidZieglerNichols::Request> req,
            std::shared_ptr<
                payload_interfaces::srv::ComputePidZieglerNichols::Response> res) {
...
void GPIOController::control_loop()
{
    while (running_) {
        ...
        const int64_t left_delta =
            (left_count - prev_left_count_) *
            static_cast<int64_t>(p.encoder.left_sign);
        const int64_t right_delta =
            (right_count - prev_right_count_) *
            static_cast<int64_t>(p.encoder.right_sign);
