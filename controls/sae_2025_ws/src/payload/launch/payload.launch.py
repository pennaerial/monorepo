...
def launch_setup(context):
    ...
    payload_params_path = os.path.join(payload_share_dir, "config", "payload_params.yaml")

    payload_name = LaunchConfiguration('payload_name').perform(context)
    payload_params_override = LaunchConfiguration('payload_params_override').perform(context)
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument('payload_params_override', default_value={}), # Used to override params when running from other launch, if necessary
