...
from px4_msgs.msg import VehicleLocalPosition, VehicleCommand, TrajectorySetpoint, OffboardControlMode
...
class WaypointMission(Mode):
    ...
    def __init__(self, node: Node, uav, waypoints: List[List[float]] = None, 
                 waypoint_tolerance: float = 0.5, speed: float = 1.0, 
                 loiter_time: float = 1.0, altitude: float = 2.0):
    ...
    def on_update(self, time_delta: float):
        ...
        if self.current_waypoint % 10 == 0:
            self.node.get_logger().info(f"Moving to waypoint {self.current_waypoint}: {target}, distance: {distance:.2f}")
...
def main(args=None):
    ...
    mission = WaypointMission(node, uav, waypoints=test_waypoints)
