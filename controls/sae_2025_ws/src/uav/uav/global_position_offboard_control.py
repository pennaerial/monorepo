...
class OffboardControl(Node):
    ...
    def __init__(self) -> None:
        ...
        self.vehicle_status_subscriber = self.create_subscription(
            VehicleStatus, '/fmu/out/vehicle_status', self.vehicle_status_callback, qos_profile)
        self.payload_detection_subscriber = self.create_subscription(
            Float32MultiArray, '/payload_tracking', self.payload_detection_callback, qos_profile)
        ...
        for angle in angles:
            ...
            y = radius * np.sin(angle)
            self.waypoints.append([x, y, self.takeoff_height])
        self.waypoints.append([0.0, 0.0, self.takeoff_height])  # Return to center

        # Create a timer for the control loop
        self.timer = self.create_timer(0.1, self.timer_callback)

    def vehicle_local_position_callback(self, msg):
        """Callback to receive the vehicle's local position."""
    ...
    def payload_detection_callback(self, msg):
        """Callback to handle payload detection."""
        if len(msg.data) >= 2:
            ...
            self.get_logger().info(f"Payload detected, hovering at {self.hover_position}")
        else:
            self.payload_detected = False
    ...
    def publish_position_setpoint(self, x: float, y: float, z: float):
        ...
        msg.timestamp = int(self.get_clock().now().nanoseconds / 1000)
        self.trajectory_setpoint_publisher.publish(msg)
        self.get_logger().info(f"Publishing setpoint: {[x, y, z]} yaw={msg.yaw:.2f}")
    ...
    def advance_to_next_waypoint(self):
        ...
        if self.current_waypoint_index < len(self.waypoints) - 1:
            self.current_waypoint_index += 1
            self.get_logger().info(f"Advancing to waypoint {self.current_waypoint_index}")
        else:
            self.mission_completed = True
            self.get_logger().info("Mission completed. Preparing to land.")
    ...
    def timer_callback(self):
        ...
        if self.payload_detected:
            # Hover at the current position if payload is detected
            self.publish_position_setpoint(*self.hover_position)
        elif not self.mission_completed:
            # Navigate to waypoints if no payload is detected
            current_waypoint = self.waypoints[self.current_waypoint_index]
            self.publish_position_setpoint(*current_waypoint)
...
def main(args=None):
    print("Starting offboard control node with payload detection...")
    rclpy.init(args=args)
