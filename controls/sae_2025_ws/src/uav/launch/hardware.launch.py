...
def generate_launch_description():
    ...
    mission = ExecuteProcess(
        cmd=['ros2', 'run', 'uav', 'mission', YAML_PATH, ','.join(vision_nodes), '--ros-args', '-r', '/image_raw:=/camera'],
        output='screen',
        emulate_tty=True,
        name='mission'
    )
    ...
    ld = LaunchDescription([
        *vision_node_actions,
        ...
        RegisterEventHandler(
            OnProcessStart(target_action=middleware, on_start=[delayed_mission, LogInfo(msg="Middleware started.")])
        ),
    ])
