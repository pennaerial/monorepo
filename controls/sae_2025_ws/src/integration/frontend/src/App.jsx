...
function DeployActionCard({
  ...
  const selectedVehicle = useMemo(
    () => fleetVehicleByName(buildSource, assignedVehicleName || ''),
    [assignedVehicleName, buildSource]
  )
  ...
  const unavailableAllowedApVehicles = useMemo(
    ...
    [allowedApVehicleNameSet, allowedApVehicles]
  )
...
function App() {
  ...
  const handleDeploySelectedSource = async () => {
    ...
    setDeployResult(null)
    try {
      const data = normalizeFleetActionResult(await api(
        '/api/fleet/deploy',
        withJsonBody(
          {
