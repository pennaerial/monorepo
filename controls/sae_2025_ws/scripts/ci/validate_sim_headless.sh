...
if [[ ! -f "$PX4_PREFIX/PINNED_COMMIT" ]]; then
    ...
    ci_log "Use the CI image or rebuild it from .github/ci/ros-jazzy-ci/Dockerfile."
    exit 1
fi
...
if [[ -n "$pinned_commit" && "$built_commit" != "$pinned_commit" ]]; then
    ...
    ci_log "Update PX4_REF in .github/ci/ros-jazzy-ci/Dockerfile and rebuild the CI image."
    exit 1
fi
