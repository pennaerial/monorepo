...
class IMU_SITL : public IMU {
   public:
    ...
    void start() override {
        ...
        node_.Subscribe(topic, &IMU_SITL::on_imu_msg, this);
    }
    ...
    void make_imu_topic(char* buf, size_t size) {
        snprintf(buf, size, "/world/%s/model/%s/link/base_link/sensor/imu_sensor/imu", sitl_cfg_.gz_world,
                 sitl_cfg_.gz_model);
    }

    /**
     * @brief callback function for imu topic. Runs in a gz background thread not managed by FreeRTOS Linux/POSIX
     * simulator
     *
